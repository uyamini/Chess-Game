...
function createBoard() {
    startPieces.forEach((startPiece, i) => {
        ...
        square.innerHTML = startPiece ? `<div id="${startPiece}" class="piece">${startPiece.toUpperCase()}</div>` : '';
...
{  
    ...
    switch(piece) {
        ...
        case 'knight' :
            ...
            case 'rook':
                ...
                case 'queen':
    // Combine the logic for rook and bishop
    // Rook logic
    ...
    // Combine rook and bishop valid moves
...
function checkForCheckmate() {
    const opponentKing = document.querySelector(`#${playerGo === 'white' ? 'black' : 'white'}-king`);
    ...
    if (isInCheck(opponentKingSquare, opponentColor)) {
        ...
        // Check if any legal move resolves the check
        const isCheckmate = legalMoves.every(move => {
            const tempSquare = move.target;
            const tempPiece = move.target.firstChild;

            // Simulate move
            move.target.appendChild(opponentKing);
            opponentKingSquare.removeChild(opponentKing);

            // Check if king is still in check after move
            const kingInCheck = isInCheck(opponentKingSquare, opponentColor);

            // Revert move
            tempSquare.removeChild(opponentKing);
            opponentKingSquare.appendChild(opponentKing);
            if (tempPiece) tempSquare.appendChild(tempPiece);

            return kingInCheck;
        });

        // If there are no legal moves that resolve the check, it's checkmate
        if (isCheckmate) {
    ...
    opponentPieces.forEach(piece => {
        ...
        for (const square of squares) {
            if (isValid(square) && pieceSquare !== square) {
                const validMove = isValid(square);
                if (validMove) {
